...
const pageMeta: Record<string, { title: string; breadcrumb: string[] }> = {
  ...
  '/propostas': { title: 'Propostas', breadcrumb: ['Gestão', 'Propostas'] },
  '/clientes': { title: 'Clientes', breadcrumb: ['Gestão', 'Clientes'] },
  '/termos': { title: 'Termos e Condições', breadcrumb: ['Configurações', 'Termos'] },
...
export function AppHeader({ onMenuClick, showMenuButton }: AppHeaderProps) {
  ...
  if (!currentPage) {
    if (location.pathname.startsWith('/clientes/')) {
      currentPage = { title: 'Detalhes do Cliente', breadcrumb: ['Gestão', 'Clientes', 'Detalhes'] };
    } else if (location.pathname.startsWith('/editar-proposta/')) {
      currentPage = { title: 'Editar Proposta', breadcrumb: ['Propostas', 'Editar'] };
    } else {
