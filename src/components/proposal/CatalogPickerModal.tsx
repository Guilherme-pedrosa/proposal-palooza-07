...
interface CatalogPickerModalProps {
  ...
  initialItemType?: 'produto' | 'servico';
}

export function CatalogPickerModal({ open, onClose, onSelect, tabelaPrecoId, initialItemType = 'produto' }: CatalogPickerModalProps) {
  const [busca, setBusca] = useState('');
  const [categoria, setCategoria] = useState<CategoriaChip>('todos');
  const [itemType, setItemType] = useState<'produto' | 'servico'>(initialItemType);
  ...
  const categoriasProduto = useMemo(
    () => categoriaChips.filter((chip) => chip.value !== 'servico'),
    []
  );

  const filtered = produtos.filter((p) => {
    const tipo = p.tipo === 'servico' ? 'servico' : 'produto';
    const termoBusca = busca.toLowerCase();
    const matchBusca = !busca
      || p.nome.toLowerCase().includes(termoBusca)
      || p.codigo?.toLowerCase().includes(termoBusca)
      || p.descricao?.toLowerCase().includes(termoBusca);
    const matchTipo = tipo === itemType;
    const matchCat = itemType === 'servico' || categoria === 'todos'
    ...
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-lg max-h-[80vh] flex flex-col">
        ...
        <div className="grid grid-cols-2 gap-2">
          <Button
            type="button"
            variant={itemType === 'produto' ? 'default' : 'outline'}
            size="sm"
            onClick={() => {
              setItemType('produto');
              setCategoria('todos');
        ...
        <div className="relative">
          ...
          <Input placeholder={itemType === 'servico' ? 'Buscar serviço...' : 'Buscar produto...'} value={busca} onChange={(e) => setBusca(e.target.value)} className="pl-9" />
        </div>
        ...
        <div className="flex-1 overflow-y-auto space-y-2 min-h-0">
          {isLoading ? (
            ...
            <p className="text-center text-sm text-muted-foreground py-8">Nenhum {itemType === 'servico' ? 'serviço' : 'produto'} encontrado</p>
          ) : (
            filtered.map((p) => {
              ...
              return (
                ...
                >
                  {p.foto_url ? (
                    ...
                    <div className="w-12 h-12 rounded bg-muted flex items-center justify-center text-lg shrink-0">{isServico ? '🛠️' : '📦'}</div>
                  )}
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-sm truncate">{p.nome}</p>
                    <p className="text-xs text-muted-foreground">
                      {isServico ? 'Serviço' : p.categoria} {p.unidade ? `· ${p.unidade}` : ''}
                    </p>
                  </div>
